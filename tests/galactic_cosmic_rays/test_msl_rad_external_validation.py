...
def test_msl_rad_external_validation_stays_honest_about_the_failed_stress_test():
    report = build_msl_rad_external_validation()

    assert report["external_alignment"]["official_overall_alignment"] > 0.6
    assert report["learned_generalizes_here"] is False
    assert report["best_mae_baselines"] == ["midpoint_0_5"]
    assert report["best_rmse_baselines"] == ["midpoint_0_5"]
    assert math.isclose(
    ...
    assert report["baselines"]["midpoint_0_5"]["mae"] < report["baselines"]["learned_calibrator_proxy"]["mae"]
